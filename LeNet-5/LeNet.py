...
class LeNet(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(
            # 卷积层C1
            nn.Conv2d(in_channels=1, out_channels=6,#in_channels为输入通道out_channels为输出通道
                      kernel_size=5, padding=2),  # 32,32*1 to 28*28*6kernel_size为卷积核大小
            nn.Sigmoid(),
            # 池化层S2
            nn.MaxPool2d(kernel_size=2, stride=2),  # 28*28*6 to 14*14*6
            # 卷积层C3
            nn.Conv2d(in_channels=6, out_channels=16, kernel_size=5),  # 10*10*16
            nn.Sigmoid(),
            # 池化层S4
            nn.MaxPool2d(kernel_size=2, stride=2),  # 10*10*16 to 5*5*16
        )
...
#作用主要是读取minist数据集时，归一化操作，它的含义是将值都转换到[-1,1]之间
# 返回值为mnist类
...
## 训练数据集的加载器，自动将数据分割成batch，shuffle函数是将所有元素随机顺序随机打乱
